...
def get_system_instruction(target_lang_name: str, is_markdown: bool = False) -> str:
    ...
    instr = "You are a professional translator for a No Man's Sky (NMS) Technology Layout Optimizer app. "
...
def get_config(
    target_lang_name: str, is_markdown: bool = False
    ...
    for attempt in range(max_retries):
        try:
            ...
            if not response.text:
                print(
                    f"Warning: Empty response for batch. Safety ratings: {response.candidates[0].safety_ratings if response.candidates else 'N/A'}"
                )
                return {}
            ...
            if (
                ...
                print(
                    f"API busy or rate limited ({ '503' if '503' in error_str else '429' }). Retrying in {wait_time}s... (Attempt {attempt+1}/{max_retries})"
                )
    ...
    for attempt in range(max_retries):
        try:
            ...
            if "429" in error_str or "503" in error_str or "UNAVAILABLE" in error_str:
                ...
                print(
                    f"Markdown API busy ({ '503' if '503' in error_str else '429' }). Retrying in {wait_time}s..."
                )
                time.sleep(wait_time)
                continue
...
def process_markdown(target_lang: str, filename: str, force: bool = False):
    ...
    if (
        ...
        and os.path.getmtime(target_file) > os.path.getmtime(source_file)
    ):
...
def main():
    parser = argparse.ArgumentParser(
        description="Translate NMS Optimizer files using Gemini AI."
    )
    parser.add_argument(
        "--lang", help="Specific language code (e.g. es). Default: all supported."
    )
    parser.add_argument(
        "--files",
        nargs="+",
        help="One or more filenames to translate (e.g. about.md translation.json).",
    )
    parser.add_argument(
        "--force", action="store_true", help="Force refresh even if already translated."
    )
