...
SKIP_FRAMES = [19]  # Frame indices to skip (0-indexed filenames, so 19 is 0019.png)
...
def create_video(files):
    ...
    try:
        ...
        for i in range(len(files) - 1):
            ...
            duration = IMAGE_DURATION + CROSSFADE_DURATION
            
            for img_idx, (img_file, out_file) in enumerate([(files[i], img1_file), (files[i+1], img2_file)]):
                cmd = [
            ...
            xfade_cmd = [
                ...
                "-i", img1_file,
                "-i", img2_file,
                "-filter_complex", f"[0:v][1:v]xfade=transition=fade:duration={CROSSFADE_DURATION}:offset={IMAGE_DURATION}[v];[v]trim=0:{duration}[trimmed]",
                "-map", "[trimmed]",
                "-c:v", "libx264",
                "-crf", "23",
                "-pix_fmt", "yuv420p",
