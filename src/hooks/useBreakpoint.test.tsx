...
describe('useBreakpoint', () => {
  ...
  beforeEach(() => {
    ...
    matchMediaSpy = vi.spyOn(window, 'matchMedia').mockReturnValue(mockMediaQueryList as MediaQueryList);
  });
  ...
  it('should remove event listener on unmount', () => {
    ...
    expect(removeEventListenerSpy).toHaveBeenCalledWith('change', addEventListenerSpy.mock.calls[0][1]);
  });

  it('should handle different breakpoints', () => {
    mockMediaQueryList.matches = false;
    const { result, rerender } = renderHook(({ bp }) => useBreakpoint(bp), { initialProps: { bp: '480px' } });
    expect(result.current).toBe(false);
