...
describe('useSeoAndTitle', () => {
  ...
  describe('Canonical Tag', () => {
    ...
    it('should remove platform and ship parameters from canonical URL', () => {
      mockUseLocation.mockReturnValue({ pathname: '/', search: '?platform=test&ship=test&other=value' });
      renderHook(() => useSeoAndTitle());
      const canonicalLink = document.querySelector('link[rel="canonical"]');
      expect(sortSearchParams(canonicalLink?.getAttribute('href') || '')).toBe(sortSearchParams('http://localhost:3000/?other=value'));
    });
    ...
    it('should keep grid parameter in canonical URL', () => {
      ...
      renderHook(() => useSeoAndTitle());
      const canonicalLink = document.querySelector('link[rel="canonical"]');
      expect(sortSearchParams(canonicalLink?.getAttribute('href') || '')).toBe(sortSearchParams('http://localhost:3000/?grid=123'));
    });
  ...
  describe('Hreflang Tags', () => {
    it('should create hreflang links for supported languages', () => {
      ...
      const enLink = Array.from(hreflangLinks).find(link => link.getAttribute('hreflang') === 'en');
      expect(sortSearchParams(enLink?.getAttribute('href') || '')).toBe(sortSearchParams('http://localhost:3000/instructions?lng=en'));

      const frLink = Array.from(hreflangLinks).find(link => link.getAttribute('hreflang') === 'fr');
      expect(sortSearchParams(frLink?.getAttribute('href') || '')).toBe(sortSearchParams('http://localhost:3000/instructions?lng=fr'));

      const esLink = Array.from(hreflangLinks).find(link => link.getAttribute('hreflang') === 'es');
      expect(sortSearchParams(esLink?.getAttribute('href') || '')).toBe(sortSearchParams('http://localhost:3000/instructions?lng=es'));

      const deLink = Array.from(hreflangLinks).find(link => link.getAttribute('hreflang') === 'de');
      expect(sortSearchParams(deLink?.getAttribute('href') || '')).toBe(sortSearchParams('http://localhost:3000/instructions?lng=de'));

      const xDefaultLink = Array.from(hreflangLinks).find(link => link.getAttribute('hreflang') === 'x-default');
      expect(sortSearchParams(xDefaultLink?.getAttribute('href') || '')).toBe(sortSearchParams('http://localhost:3000/instructions?lng=en'));
    });
    ...
    it('should update existing hreflang links', () => {
      ...
      expect(sortSearchParams(existingEnLink.getAttribute('href') || '')).toBe(sortSearchParams('http://localhost:3000/about?lng=en'));
      expect(sortSearchParams(existingFrLink.getAttribute('href') || '')).toBe(sortSearchParams('http://localhost:3000/about?lng=fr'));
      expect(sortSearchParams(existingEsLink.getAttribute('href') || '')).toBe(sortSearchParams('http://localhost:3000/about?lng=es'));
      expect(sortSearchParams(existingDeLink.getAttribute('href') || '')).toBe(sortSearchParams('http://localhost:3000/about?lng=de'));
      expect(sortSearchParams(existingXDefaultLink.getAttribute('href') || '')).toBe(sortSearchParams('http://localhost:3000/about?lng=en'));
    ...
    it('should remove platform and grid parameters from hreflang URLs', () => {
      mockUseLocation.mockReturnValue({ pathname: '/', search: '?platform=test&grid=123&other=value' });
      renderHook(() => useSeoAndTitle());
      ...
      expect(sortSearchParams(enLink?.getAttribute('href') || '')).toBe(sortSearchParams(expectedEnUrl.toString()));
      expect(sortSearchParams(frLink?.getAttribute('href') || '')).toBe(sortSearchParams(expectedFrUrl.toString()));
      expect(sortSearchParams(esLink?.getAttribute('href') || '')).toBe(sortSearchParams(expectedEsUrl.toString()));
      expect(sortSearchParams(deLink?.getAttribute('href') || '')).toBe(sortSearchParams(expectedDeUrl.toString()));
    });
    ...
    it('should correctly generate x-default hreflang tag', () => {
      ...
      expect(sortSearchParams(xDefaultLink?.getAttribute('href') || '')).toBe(sortSearchParams(expectedUrl.toString()));
    });
  });
});